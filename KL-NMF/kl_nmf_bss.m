function [x, s, fs, bits] = kl_nmf_bss(audio_file, M, N, K, iterations)
...
% calculate complex & amplitude spectrograms
C = zeros(M, N);
X = zeros(M, N);
...
% amplify scale
X = X / mean(mean(X)) * 1000;

% initialize parameters
[cost, W, H, Y, XiY] = kl_nmf_init(X, K);
fprintf(1, 'cost[%d] = %f\n', length(cost), cost(length(cost)));

% update parameters
for it = 1 : iterations
  [cost, W, H, Y, XiY] = kl_nmf_train(cost, W, H, Y, XiY, X);
  fprintf(1, 'cost[%d] = %f\n', length(cost), cost(length(cost)));
end
